// FPA V1.0.2 (Beta: Listener Fix) | Staging
...
const AI_ENGINE_REGEX = /(\.|\/)(chatgpt|gemini.google|perplexity|claude|copilot.microsoft)\.(com|net|org|ai|co\.[a-z]{2}|com\.[a-z]{2})\//;
const AI_ENGINE_DOMAINS = ['chatgpt', 'gemini', 'perplexity', 'claude', 'copilot'];
...
const SEARCH_ENGINE_DOMAINS = ['google', 'bing', 'yahoo', 'duckduckgo', 'ecosia', 'yandex', 'baidu', 'startpage', 'ask', 'seznam', 'naver'];
...
// Review site referring
const REVIEW_SITE_REGEX = /(\.|\/)(g2|capterra|shortlister|sourceforge|selectsoftwarereviews|softwareselect|peoplemanagingpeople)\.(com|net|org|co\.[a-z]{2}|com\.[a-z]{2})\//;
const REVIEW_SITE_DOMAINS = ['g2', 'capterra', 'shortlister', 'sourceforge', 'selectsoftwarereviews', 'softwareselect', 'peoplemanagingpeople']
...
function updateUserLevelData() {
	...
	try {
		window.fpaData.ga_cid = Cookies.get('_ga', { domain: DOMAIN }) || window.fpaData.ga_cid;
		window.fpaData.hsu_id = Cookies.get('hubspotutk', { domain: DOMAIN }) || window.fpaData.hsu_id;
	} catch (e) {
	...
	if (window.fpaData.lact) {
		const sessionExpired = Date.now() - window.fpaData.lact > SESSION_TIMEOUT_MINUTES * 60 * 1000;
...
function updateSessionLevelData() {
	...
	if (!window.fpaData.ses[0].sid) {
		...
		window.fpaData.ses[0].ldp = window.fpaData.ses[0].ldp || window.location.pathname;
...
function populateAttrValues() {
	...
	window.fpaData.ses[0].attr.src = window.fpaData.ses[0].attr.src || urlParams.get('utm_source');
	window.fpaData.ses[0].attr.med = window.fpaData.ses[0].attr.med || urlParams.get('utm_medium');
	window.fpaData.ses[0].attr.cmp = window.fpaData.ses[0].attr.cmp || urlParams.get('utm_campaign');
	window.fpaData.ses[0].attr.trm = window.fpaData.ses[0].attr.trm || urlParams.get('utm_term');
	window.fpaData.ses[0].attr.kwd = window.fpaData.ses[0].attr.kwd || urlParams.get('utm_keyword');
	window.fpaData.ses[0].attr.cnt = window.fpaData.ses[0].attr.cnt || urlParams.get('utm_content');
...
function checkChannelAttribution() {
	...
	const isPaidChannel = window.location.href.includes('/lp/');
	if (AI_ENGINE_REGEX.test(document.referrer)) {
		window.fpaData.ses[0].attr.med = window.fpaData.ses[0].attr.med || isPaidChannel ? 'paid-ai' : 'organic-ai';
		window.fpaData.ses[0].attr.src = window.fpaData.ses[0].attr.src || extractDomain(document.referrer, AI_ENGINE_DOMAINS);
	} else if (SEARCH_ENGINE_REGEX.test(document.referrer)) {
		window.fpaData.ses[0].attr.med = window.fpaData.ses[0].attr.med || isPaidChannel ? 'paid-search' : 'organic-search'
		window.fpaData.ses[0].attr.src = window.fpaData.ses[0].attr.src || extractDomain(document.referrer, SEARCH_ENGINE_DOMAINS);
	} else if (SOCIAL_MEDIA_REGEX.test(document.referrer)) {
		window.fpaData.ses[0].attr.med = window.fpaData.ses[0].attr.med || isPaidChannel ? 'paid-social' : 'organic-social';
		window.fpaData.ses[0].attr.src = window.fpaData.ses[0].attr.src || extractDomain(document.referrer, SOCIAL_MEDIA_DOMAINS);
	} else if (REVIEW_SITE_REGEX.test(document.referrer)) {
		window.fpaData.ses[0].attr.med = window.fpaData.ses[0].attr.med || isPaidChannel ? 'paid-review-site' : 'organic-review-site';
		window.fpaData.ses[0].attr.src = window.fpaData.ses[0].attr.src || extractDomain(document.referrer, REVIEW_SITE_DOMAINS);
	} else {
		window.fpaData.ses[0].attr.med = window.fpaData.ses[0].attr.med || 'web-referral';
		window.fpaData.ses[0].attr.src = window.fpaData.ses[0].attr.src || document.referrer;
	}
...
function populateAdsValues() {
	...
	window.fpaData.ses[0].ads.gclid = window.fpaData.ses[0].ads.gclid || urlParams.get('gclid');
	window.fpaData.ses[0].ads.msclkid = window.fpaData.ses[0].ads.msclkid || urlParams.get('msclkid');
	window.fpaData.ses[0].ads.li_fat_id = window.fpaData.ses[0].ads.li_fat_id || urlParams.get('li_fat_id');
	window.fpaData.ses[0].ads.fbclid = window.fpaData.ses[0].ads.fbclid || urlParams.get('fbclid');
...
try {
	if (document.readyState == 'loading') {
		document.addEventListener('DOMContentLoaded', runFpaAnalytics);
	} else {
		runFpaAnalytics();
	}
} catch (e) {
